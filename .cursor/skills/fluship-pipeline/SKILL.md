...
Long ids (`buildAab`, `buildApk`, `buildSplits`, `podInstall`, `buildIpa`, `distPlayProduction`, `distPlayInternal`, `distAppStore`, `distDrive`) run through `dart tool/pipeline_wait.dart` so every poll waits 30 seconds, never 60s then 90s then 120s. Flutter builds use `--dir {targetProjectPath}`; store uploads use the Fluship workspace cwd. Other slow jobs and the picker run in the background and get polled in 5 second checks until the footer shows `exit_code`. Never guess one long duration.
