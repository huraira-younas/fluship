...
Once the picker returns, stop thinking. Every job is run command, paste board, stop. A hook starts the next job. Reasoning is only for a command that failed or a value that is missing.
...
Paste the board into your own reply after every job, in a fenced code block, then stop. Running `pipeline_progress.dart` is not printing it: tool output sits in a collapsed block, and thinking is never shown. Never put the board in thinking, and never start the next job in the same turn. The user must read NOW / DONE / WAIT / FAIL / SKIP as chat text every time.
