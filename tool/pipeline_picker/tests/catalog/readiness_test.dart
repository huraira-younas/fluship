...
import '../../catalog/cache_io.dart';
import '../../catalog/catalog.dart';
import '../../catalog/readiness.dart';
...
void main() {
  ...
  _cacheWhatsApp();
  _noEmDash();
  stdout.writeln('pipeline catalog tests: ok');
}
...
void _noEmDash() {
  ...
  final tool = pipelineToolRoot();
  final repo = Directory(tool).parent.path;
  final files = [
    ...Directory(
      pathJoin(tool, 'pipeline_picker'),
    ).listSync(recursive: true).whereType<File>(),
    ...Directory(tool).listSync().whereType<File>(),
    File(pathJoin(repo, 'AGENTS.md')),
    File(pathJoin(repo, '.cursor', 'skills', 'fluship-pipeline', 'SKILL.md')),
  ];
